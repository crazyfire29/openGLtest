...
class TextFieldRectangle(Shape):
    ...
    def draw(self):
        if self.get_visible():
            ...
            x = (self.vertices[1][0] + self.local_translation[0] + self.global_translation[0] - text_width) // 2
            y = (self.vertices[2][1] + self.local_translation[1] + self.global_translation[1] - text_height) // 2

            self.canvas.delete("all")

            self.canvas.create_text(x, y, text=self.text, fill=text_color, font=("TkDefaultFont", 24))